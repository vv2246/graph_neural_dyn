    ...
    for model in ["SAGEConv_single",   "GraphConv_single", "ResGatedGraphConv_single", "GATConv_single","ChebConv_single" ]:
        ...
        for (test_with_new_graph, test_dist) in [(False, False), (False, True), (True, False)]:
            with torch.no_grad():
                ...
                for i in range(g_pred.shape[1]):
                    pred_yi = g_pred[:,i].squeeze()
                    true_yi = g[:,i].squeeze()
                    li = sum(abs(pred_yi - true_yi))/sum(abs(true_yi))
                    loss_tot.append(float(li))
                loss_tot_scaled= torch.tensor(loss_tot) * 100
                loss = float(torch.mean(loss_tot_scaled))
                loss_err = float(loss_tot_scaled.std())
            if (test_with_new_graph == True )and ( test_dist== False):
                print("$", round(loss,2),"\\pm" ,  round(loss_err,2),"$", end = "")
            else:
                print( "$",round(loss,2),"\\pm" ,  round(loss_err,2),"$", end = " & ")
