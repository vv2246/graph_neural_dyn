...
if __name__ == "__main__":
    ...
    color_null, color_test = "blue","darkorange"
    line_null, line_test = "-","--"
    ...
    g = nx.read_gml(f"{network_name}.gml")
    ...
    y_pred = odeint(lambda y, t: nn_list[0](y, t, L_test), x0_test[:,None], t, method="dopri5").squeeze().detach()
    

    dyn_test =  Dynamics(A=A_test,   model = params.dynamics_name)
    sol = odeint( dyn_test, x0_test, t, method="dopri5")

    i = 137
    fig,ax = plt.subplots(nrows = 2, figsize= (15,6),sharex= True)
    ax[0].plot(t,sol[:,i,0], color = color_null, linestyle = line_null, lw = 2,alpha =1, label = "True")
    ax[1].plot(t,sol[:,i,1],  color = color_null, linestyle = line_null, lw = 2,alpha = 1)
    ax[0].plot(t, y_pred[:,i,0], color = color_test, lw = 2, linestyle = line_test, label="Predicted")
    ax[1].plot(t, y_pred[:,i,1], color = color_test, lw = 2, linestyle = line_test)
    ...
    ax[1].set_xlim(0,100)
    ax[0].legend(loc=1)
    plt.savefig(f"node_{i}_timeseries.pdf")
    plt.show()
    
    fig,ax = plt.subplots(nrows = 1, figsize= (6,6),sharex= True)
    ax.plot(sol[:,i,0], sol[:,i,1],label="True", color = color_null, linestyle = line_null, lw = 2,alpha =1)
    ax.plot(y_pred[:,i,0], y_pred[:,i,1],color = color_test, lw = 2, linestyle = line_test,label="Predicted")
    ax.set_xlabel("$x_{i,0}(t)$")
    ax.set_ylabel("$x_{i,1}(t)$")
    ax.set_ylim(-2,2.5)
    ax.set_xlim(-2.5,2)
    # ax.set_yticks(np.linspace(-2,2.5,5))
    # ax.set_xticks(np.linspace(-2.5,2,5))
    ax.legend(loc=1)
    plt.tight_layout()
    # plt.savefig(f"node_{i}_phase_portrait.pdf")
    plt.show()
    ...
    d_stat_test = np.stack(compute_d_statistics(list_of_experiments = nn_list,  x_test = y_pred , M = 5, 
                                              direct_fun = True, number_of_draws = 1000, A = A,n_id = i))
    ...
    ax , ax2 = axs
    sns.ecdfplot(d_stat_test1, complementary= True, color = color_test, linewidth=2,alpha= 1, ax = ax,linestyle = line_test,label="Test")
    sns.ecdfplot(d_stat_full1, complementary = True, color = color_null, linewidth=2,alpha= 1,ax = ax,linestyle =line_null,label="Null")
    ...
    sns.ecdfplot(d_stat_test2, complementary= True, color = color_test, linewidth=2,alpha= 1, ax = ax2,linestyle = line_test)
    sns.ecdfplot(d_stat_full2, complementary = True, color = color_null, linewidth=2,alpha= 1,ax = ax2,linestyle = line_null)
    ...
    plt.tight_layout()
    # plt.savefig(f"d_statistic_node_{i}.pdf")
    plt.show()
