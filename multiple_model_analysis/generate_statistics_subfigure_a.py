...
if __name__ == "__main__":
    ...
    for i in range(len(name_dynamics_list)):
        fname = f"results/delta_vs_pval_vs_loss_network_size_{network_size}_new_network_{new_network}_alpha_{alpha_sig}.csv"
    ...
    # if size != 15:
        df1 = pd.DataFrame([loss_list], columns = ["Dynamics", "loss_mean","loss_std"])
        if os.path.isfile(fname1) == True  and i != 0:
            df1.to_csv(fname1,header = False, mode = "a+")
        else:
            df1.to_csv(fname1,header = True)
                
        res_all = []
        for delta in np.linspace(0,1,11):
            print(delta)
            res =  generate_statistics(A_test = A, delta_test = delta, true_dyn = dyn,
                                       d_stat_full_sample = d_stat_full_sample ,
                                       time_array = time ,  neural_net_list = nn_list , M= M ,  
                                       alpha_sig= 0.05, number_of_iterations =10  )
            res_all.append(res)
